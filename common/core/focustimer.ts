...
export async function getFocusSessionsFromLocalStorage(): Promise<
  GetFocusSessionResponse[] | null
> {
  return await storage.getItem<GetFocusSessionResponse[]>(
    FOCUS_SESSION_STORAGE_KEY,
  );
}

export async function setFocusSessionsToLocalStorage(
  focusSessions: GetFocusSessionResponse[],
): Promise<void> {
...
export async function setNextFocusSessionToLocalStorage(
  focusSession: GetFocusSessionResponse | null,
): Promise<void> {
